...
Node *create_node(char *word, int count, int filenum) {
	Node *new_node;
	// memory allocation for new_node
	if ((new_node = malloc( sizeof(Node))) == NULL) {
		perror("create_node:");
		exit(1);
	}
    // assign a word to this node
	strncpy(new_node->word, word, MAXWORD);
	// make sure it is null terminated
	new_node->word[MAXWORD-1] = '\0';
        /* memset is a function to fill memory with a constant byte
           used to initialize memory
        */
    // initialize initial values
	memset(new_node->freq, 0, MAXFILES * sizeof(int));
	new_node->freq[filenum] = count;
	new_node->next = NULL;
	return new_node;
}
...
Node *add_word(Node *head, char **filenames, char *word, char *fname) {
    ...
    // make sure this list is not empty, and this node should be added
    // as a head to this linked-list
	...
	while(cur != NULL) {
		...
		if((strcmp(cur->word, word)) == 0) {
			// inceament the frequency
			cur->freq[filenum] += 1;
			return head;
		// there is no Node contains this word, so add this word into
		// this linekd-list
    ...
    // assume this linked-list is not empty and 
...
void display_list(Node *head, char **filenames) {
	...
	while (cur != NULL) {
		...
		for (i = 0; i < MAXFILES; i++) {
			// not reaching the end of filenames list
			if (filenames[i] != NULL) {
				printf("    %d %s ", cur->freq[i], filenames[i]);
			// reach the end of filenames list
			} else {
				printf("\n");
				break;
			}
